...
public class Hello {
    ...
    public static void main(String[] args) {
        ...
        // 从List中获取到值时，List的类型是混合型，需要具体指定类型，和强制类型转换。如果是制定了具体类型，就不需要在获取元素时进行强制转换
...
- 普通下标循环取值
- `for (Object item : xxx)`取每个元素的值，迭代循环取值
...
在python中就要特别注意是有序还是无序，python中set的无序的，碰到HashSet时，直接用set来模拟。碰到TreeSet在python中用先set完在sorted手动排序
...
public class Hello {
    ...
    public static void main(String[] args) {
        ...
        // s2 = {"东京热","东北热","南京热"}
        HashSet s2 = new HashSet(){  //简化写法，连着实例化对象，添加元素
            {
                add("东京热");
                add("东北热");
                add("南京热");
        ...
        System.out.println(s4); // [A站, B站, P站]，自动对元素进行排序了
...
public class Hello {
    ...
    public static void main(String[] args) {
        ...
        HashSet v1 = new HashSet(); // 空  -> [B站, A站]
        v1.addAll(s1);  // 将s1中所有内容加入到v1中
        v1.retainAll(s2); // 交集 & ，v1中只剩下交集的结果
        System.out.println(v1);
...
- TreeMap，默认根据key排序。（常用）
  ...
  看到java代码Map的添加顺序 ，不必在意，如果用的是TreeMap后续会自动排序，所以先将java中Map打印出来看顺序，在python中根据key值排序后，再拼接操作。
...
public class Hello {
    ...
    public static void main(String[] args) {
        ...
        // 循环: 示例1，指定类型并拿到实体，只有Map才有实体，用于循环取值
        // {  ("name", "alex")，("age", "18"),  }，相当于python的字典的items()方法
...
就近原则找成员函数中用到的变量 ，方法在父类中，就先找父类中的值，没有再找子类中。方法在子类中，就在子类中先找变量值，没有再去父类中查找。
...
找到一个方法，其传递的值类型是一个泛指类型，是接口的泛指或者是父类的泛指，有三种方法来确定其是那个具体的类：

- 寻找谁实现了或继承了泛指类型，适用于实现的不多的情况，使用hook机制来找到
- 寻找调用栈，一级一级往上找
- 可以在该方法中hook，然后输出该参数的类型来找到具体的类
